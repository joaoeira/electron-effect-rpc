...
stop() must: 1. Remove all stream method handlers (ipc.removeHandler for each stream/ channel) 2. Remove the cancel handler (ipc.removeHandler for stream-cancel channel) 3. Interrupt all active Fibers imperatively:
