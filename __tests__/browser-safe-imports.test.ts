 ...
 *
 * kit.ts must only dynamic-import preload.ts inside the `.preload()` method
 * so that consumers in browser-like environments (Vitest browser mode, etc.)
 * can import `createIpcKit` without pulling in electron.
 */
describe("browser-safe imports", () => {
  ...
  it("compiled kit.js has no static import/require of preload", () => {
